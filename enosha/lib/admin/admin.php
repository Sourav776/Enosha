...
require_once './backup/backup_info.php';
		...
		$com_mes=isset($_SESSION['com_mes']);
		$com_mes_format=isset($_SESSION['com_mes_format']);
		$com_mes_lang=isset($_SESSION['com_mes_lang']);
		$com_mes_act=isset($_SESSION['com_mes_act']);
		$com_mes_local=isset($_SESSION['com_mes_local']);
		$exist_cata=isset($_SESSION['exist_mes']);
		$exist_ex=isset($_SESSION['exist_mes_ex']);
		$exist_lan=isset($_SESSION['exist_mes_lan']);
		$exist_act=isset($_SESSION['exist_mes_act']);
		$exist_local=isset($_SESSION['exist_mes_local']);
		$com_mod = isset($_SESSION['com_mes_mod']);
		$backup_mes=isset($_SESSION['backup_mes']);
		if(isset($_SESSION['user_display_cata'])==""){
			$userDisplay_cata="none";
		}else{
			$userDisplay_cata=isset($_SESSION['user_display_cata']);
		}
		if(isset($_SESSION['user_display_format'])==""){
			$userDisplay_format="none";
		}else{
			$userDisplay_format=isset($_SESSION['user_display_format']);
		}
		if(isset($_SESSION['user_display_lang'])==""){
			$userDisplay_lang="none";
		}else{
			$userDisplay_lang=$_SESSION['user_display_lang'];
		}
		if(isset($_SESSION['user_display_act'])==""){
			$userDisplay_act="none";
		}else{
			$userDisplay_act=$_SESSION['user_display_act'];
		}
		if(isset($_SESSION['user_display_target'])==""){
			$userDisplay_target="none";
		}else{
			$userDisplay_target=$_SESSION['user_display_target'];
		}
		if(isset($_SESSION['user_display_mod'])==""){
			$userMod_target="none";
		}else{
			$userMod_target=$_SESSION['user_display_mod'];
		}
		if(isset($_SESSION['backup_mes'])==""){
  ...
  <div id="con">
    <div id="inner">
      <div class="left floatl3"><!-- InstanceBeginEditable name="EditRegion8" -->
        <!-- ////////////////////////////////////////////////////////////////////////////////////////////////////// -->
       <body onload="init()">
        <p><a href="../../index.php"><?php echo $home ?></a> <img src="../../images/insert.png" alt="link" width="12" height="11" /> <?php echo $administration ?></font></p>
        <p align="right">
          <?php logged_in_user() ?>
        </p>
