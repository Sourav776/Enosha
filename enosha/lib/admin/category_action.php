...
if ($_POST['pass_cata'] =='cat_add'){
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if (!$cat_array[strtolower($category)]>0){
				...
				if (!is_dir("$upload_path/$last_cat_id")) mkdir("$upload_path/$last_cat_id", 0777);
				if (is_dir("$upload_path/$last_cat_id")){
...
if ($_POST['pass_cata'] =='lev1_add')
{
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if (!$lev1_array[strtolower($level1)]>0){
				...
				if (!is_dir("$upload_path/$super/$last_subcat_id")) mkdir("$upload_path/$super/$last_subcat_id", 0777);
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if($lev2_array['$level2']!=$super){
				if (!$lev2_array[strtolower($level2)]>0){
					...
					$last_subcat_id=mysql_insert_id();	
					if (!is_dir("$upload_path/$super2/$super/$last_subcat_id")) mkdir("$upload_path/$super2/$super/$last_subcat_id", 0777);
					$_SESSION['com_mes']= $new_subcategory_added;
					header('Location:admin.php');	
				}else
					$_SESSION['exist_mes']= $subcategory_already_exists;
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if($lev3_array['$level3']!=$super){
				if (!$lev3_array[strtolower($level3)]>0){
					...
					$last_subcat_id=mysql_insert_id();	
					if (!is_dir("$upload_path/$super2/$super1/$super/$last_subcat_id")) mkdir("$upload_path/$super2/$super1/$super/$last_subcat_id", 0777);
					$_SESSION['com_mes']= $new_subcategory_added;
					header('Location:admin.php');	
				}else
					$_SESSION['exist_mes']= $subcategory_already_exists;
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if($lev4_array['$level4']!=$super){
				if (!$lev4_array[strtolower($level4)]>0){
					...
					$last_subcat_id=mysql_insert_id();	
					if (!is_dir("$upload_path/$super3/$super2/$super1/$super/$last_subcat_id")) mkdir("$upload_path/$super3/$super2/$super1/$super/$last_subcat_id", 0777);
					$_SESSION['com_mes']= $new_subcategory_added;
					header('Location:admin.php');	
				}else 
					$_SESSION['exist_mes']= $subcategory_already_exists;
	...
	while ($rows=mysql_fetch_array($result)){
			...
			if($lev5_array['$level5']!=$super){
				if (!$lev5_array[strtolower($level5)]>0){
					...
					$last_subcat_id=mysql_insert_id();
					if (!is_dir("$upload_path/$super4/$super3/$super2/$supe1/$super/$last_subcat_id")) mkdir("$upload_path/$super4/$super3/$super2/$supe1/$super/$last_subcat_id", 0777);
					$_SESSION['com_mes']= $new_subcategory_added;
					header('Location:admin.php');	
				}else 
					$_SESSION['exist_mes']= $subcategory_already_exists;
